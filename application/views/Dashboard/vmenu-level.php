        ...
        <div class="portlet-body">
          ...
          <table class="table table-striped table-bordered table-hover">
           ...
           <?php foreach ($data as $key) : ?>
             <tr class="tr<?=$key['level']?>">
              <td class="center level"><?=$key['level']?></td>
              <td class="center nama"><?=$key['nama']?></td>
              <td class="center kategori"><?=$key['kategori'] ?></td>
              <td class="center deskripsi"><?=$key['deskripsi']?></td>
              <td class="center">
              <?php if($key['level'] > 4): ?>
                <button class="btn btn-primary" onclick="edit('<?=$key['level']?>')"><i class="fa fa-pencil"></i></button>
...
<script type="text/javascript">
  function edit(level)
  {
    var nama      = $(".tr"+level+" .nama").html();
    var deskripsi = $(".tr"+level+" .deskripsi").html();
    var kategori  = $(".tr"+level+" .kategori").html();

    $("input[name='level']").val(level);
    $("input[name='nama']").val(nama);
    $("select option[value='"+kategori+"']").prop("selected","true");
    $("textarea[name='deskripsi']").val(deskripsi);
    $("input[name='tb']").val("Ubah");
    $("form").prop("action", "<?=base_url()?>p/role/level_edit");
  }
  ...
  $("button#batal").click(function(){
    ...
    $("input[name='tb']").val("Tambah");
    $("form").prop("action", "<?=base_url()?>p/role/level_add");
  });
</script>
